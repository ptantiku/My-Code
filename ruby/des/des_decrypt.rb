...
options = {
	:verbose => false,
	:cipher => "a043d2c67a571e54",
	:key => "itisakey",
	:format => :char
}
opts = OptionParser.new
opts.banner = "Usage: des_decrypt.rb [options] DES_cipher"
opts.separator ""
...
opts.on('-k KEY', '--key KEY', String, 'Key for decryption') do |k|
	options[:key] = k
end
opts.on("-c CIPHERTEXT", "--ciphertext CIPHERTEXT", String, 'Ciphertext before decryption') do |p|
	options[:cipher] = p
end
opts.on("-i", "--iteration", 'Display output of each iteration') do |i|
	options[:iteration] = i
end
...
opts.on('-d', '--delimiter', String, 'Display delimiter between bytes') do |d|
	options[:whitespace] = d
end
opts.parse!
...
options[:cipher] = ARGV[0]
final_output = ""
puts "Input:  #{options[:cipher]}" if options[:verbose]

#convert input cipher-text to blocks
cipher = options[:cipher].scan(/[0-9a-fA-F]{2}/).map{|h| '%08b' % h.to_i(16)}.join
cipher_block = cipher.scan(/[01]{1,64}/)
puts cipher_block if options[:verbose]

#generate 16 Key blocks
#key generation
key_binary = options[:key].unpack('B64')[0]
puts "Key: " + key_binary.scan(/.{8}/).map{|i| i.to_i(2).to_s(16)}.join if options[:verbose]
...
16.times do |round|
	...
	#Second permutation (PC2)
	key_before_pc2 = key_left + key_right
	key_after_pc2 = PC2.map{|i| key_before_pc2[i-1]}.join
	key_blocks[round] = key_after_pc2
end

cipher_block.each do |input_block|
	...
	16.times do |round|
		...
		#getting key block in reverse order (for decryption)
		key_after_pc2 = key_blocks.reverse[round]		
